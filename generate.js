...
// generated by make-video-thumbs.mjs). Google will NOT index a video without a
// thumbnail, so VideoObject schema, the <video poster>, and the video sitemap
// entries are only emitted for slugs whose thumbnail file exists. This keeps the
// build valid whether or not thumbnails have been generated yet.
const THUMB_DIR  = path.resolve('assets/video-thumbs');
const hasThumb   = (slug) => { try { return fs.existsSync(path.join(THUMB_DIR, `${slug}.jpg`)); } catch { return false; } };
const thumbUrl   = (slug) => `${SITE_URL}/assets/video-thumbs/${slug}.jpg`;
const VIDEO_DATE = TODAY; // VideoObject uploadDate
...
function buildExercisePage(ex, allInCat, prevEx, nextEx) {
  ...
  const videoSlot = buildVideoSlot(ex.video, ex.slug);
  ...
  // JSON-LD VideoObject — only when a real thumbnail exists (Google requires one).
  const videoJsonLd = (ex.video && hasThumb(ex.slug)) ? `{
    "@context":"https://schema.org",
    "@type":"VideoObject",
    "name":${JSON.stringify(ex.name + ' — Off-Ice Hockey Exercise Demo')},
    "description":${JSON.stringify(descMeta)},
    "thumbnailUrl":${JSON.stringify(thumbUrl(ex.slug))},
    "contentUrl":${JSON.stringify(ex.video)},
    "uploadDate":${JSON.stringify(VIDEO_DATE)}
  }` : '';
...
function buildVideoSlot(url, slug) {
  ...
  if (/\.mp4/i.test(url)) {
    const poster = (slug && hasThumb(slug)) ? ` poster="${escHtml(thumbUrl(slug))}"` : '';
    return `<video class="video-embed" src="${escHtml(url)}"${poster} controls playsinline preload="metadata"></video>`;
  }
  return placeholder();
}
...
// Static (hand-maintained) URLs that are NOT generated from the exercise sheet.
// IMPORTANT: keep this list in sync when you add/remove top-level pages or
...
const STATIC_PAGES = [
  ...
  { loc: '/program-19-39.html',                                    freq: 'monthly', pri: '0.9' },
  { loc: '/program-40-plus.html',                                  freq: 'monthly', pri: '0.9' },
  { loc: '/articles.html',                                         freq: 'weekly',  pri: '0.8' },
  { loc: '/off-ice-hockey-training.html',                          freq: 'monthly', pri: '0.9' },
  ...
  { loc: '/library.html',                                          freq: 'weekly',  pri: '0.8' },
];
